...
function getAverage(...theArgs) {
  ...
  return theArgs.reduce((acc, el) => acc + el / theArgs.length, 0);
}
...
function roundToPowerOfTen(num, pow) {
  // throw new Error('Not implemented');
  /* if(num%(10*pow) < 5*10*pow){

  } */

  return Math.trunc(Math.round(num / 10 ** pow)) * 10 ** pow;
}
...
function isPrime(n) {
  // throw new Error('Not implemented');
  for (let i = 2; i <= Math.trunc(Math.sqrt(n)); i += 1) {
    if (n % i === 0) {
      return false;
    }
  }
...
function getSumOfDigits(num) {
  // throw new Error('Not implemented');
  const sum = `${num}`.split('').reduce((acc, el) => acc + el * 1, 0);
  return sum;
}
...
function getSumOfNumbers(...args) {
  ...
  return args.reduce((acc, el) => acc + el, 0).toFixed(1);
}
